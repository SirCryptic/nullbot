  ...
  fs.readFile(`./data/serverdata/${message.guild.id}/litemode.txt`, function (err, litedata) {
    ...
    var optionerror = new Discord.RichEmbed()
        ...
        .setDescription('**Shorten URL**\n' +
                        '`url shorten https://nullsec.online/`\n' +
                        '**Expand URL**\n' + 
                        '*Coming Soon :tm:*')
    ...
    var userurlerror = new Discord.RichEmbed()
       ...
       if(option.includes('expand')) {
                    ...
                    .setDescription('Sadly, Google is starting to end support for the `goo.gl` URL Shortening service and transitioning to the FireBase Dynamic Link API\n' +
                                    ...
                                    'At this time, the Dynamic Link API is only for Android and iOS apps and manual URL creation. This means that for NULLBot, which is programmed in NodeJS, cannot shorten or expand URLs with Google\n' +
                                    '**What\'s going to happen to Nullbot?**\n' +  
                                    'Nothing For now,defaulted to the TinyURL API, but it can only shorten URLs not expand them.')
